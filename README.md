...
## Daily news brief (8 AM)

Unlike the lessons (pre-baked weekly), the news must be **fresh daily**, so it's
generated by a **scheduled Claude task** that runs each morning, web-searches
Singapore politics + major overseas economic events, writes a short brief, and
POSTs it to `/api/news`, which relays it to Telegram (bot token stays in Vercel).

- `api/news.js` — the relay. `POST /api/news?key=<CRON_SECRET>` with `{"text": "<html>"}`.
  Test it: `curl -X POST "<SITE_URL>/api/news?key=<CRON_SECRET>" -H "content-type: application/json" --data '{"text":"🗞 test brief"}'`
- The scheduled task holds only `CRON_SECRET` (not the bot token). Its prompt has a
  `__CRON_SECRET__` placeholder that must be replaced with the real value before it
  can send.
- No extra env vars — reuses `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, `CRON_SECRET`.
